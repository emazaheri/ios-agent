...
async def test_opening_an_app_hands_the_agent_what_was_written_about_it() -> None:
    """The whole path, not just the tool: the app list is fetched, the name is
    resolved to a bundle id the way the session resolves it, and the file for
    that id lands in the reply the model reads.

    Asserted end to end because every seam here is one nothing else covers.
    `run_goal` fetches the installed apps for the opening turn and used to
    throw the bundle ids away, keeping only the names; a briefing keyed on a
    bundle id cannot be found without them.
    """
    model = DeviceModel()
    session, _, _ = build_session(model, _settings())
    scripted = ScriptedModel(
    ...
    outcome = await run_goal(session, "Open Cards.", model=scripted)

    assert "Notes on this app" in tool_replies(scripted)
    assert "Nudges" in tool_replies(scripted), "the file for com.example.cards was not the one read"
    assert outcome.skill_tokens > 0, "the briefing was not charged to the run"


async def test_an_app_nobody_wrote_about_adds_nothing_to_the_reply() -> None:
    """The ordinary case. Two files exist and a phone holds a hundred apps."""
    model = DeviceModel()
    session, _, _ = build_session(model, _settings())
    scripted = ScriptedModel(
        [
    ...
    assert "Notes on this app" not in tool_replies(scripted)
    assert outcome.skill_tokens == 0
